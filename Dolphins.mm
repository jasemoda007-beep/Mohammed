...
static long gWorld(){
    ...
    if ([BundID containsString:@"ig"]) {
        gWorldaddr = 0x102A8D3C4;
        gWorldData = 0x10A66BFE0;
    } if ([BundID containsString:@"kr"]) {
        gWorldaddr = 0x102B18D38;
        gWorldData = 0x10A7EDA60;
    }if ([BundID containsString:@"rekoo"]) {
        gWorldaddr = 0x102B2C044;
        gWorldData = 0x10A824360;
    }if ([BundID containsString:@"vn"]) {
        gWorldaddr = 0x1028F8044;
        gWorldData = 0x10A576D70;
    }if ([BundID containsString:@"imobile"]) {
        gWorldaddr = 0x10238BF30;
        gWorldData = 0x109C3FE60;
    }
...
static long gName(){
    ...
    NSString *BundID = [infoDictionary objectForKey:@"CFBundleIdentifier"];
    if ([BundID containsString:@"ig"]) {
        gNameaddr = 0x10487CBE8;
        gNameData = 0x109FCAAA0;
    } if ([BundID containsString:@"kr"]) {
        gNameaddr = 0x1049EAB1C;
        gNameData = 0x10A14B000;
    }if ([BundID containsString:@"rekoo"]) {
        gNameaddr = 0x1049EAB1C;
        gNameData = 0x10A1827C0;
    }if ([BundID containsString:@"vn"]) {
        gNameaddr = 0x1047CACD4;
        gNameData = 0x109FCA750;
    }if ([BundID containsString:@"imobile"]) {
        gNameaddr = 0x104256E0C;
        gNameData = 0x1095A5840;
}
...
//            int objectCount = memoryTools.readInt(uLevel + PubgOffset::ULevelParam::ObjectCountOffset);
            ...
            for (int index = 0; index < objectCount; ++index) {
                ...
                if (strstr(className.c_str(), "PlayerPawn") || (strstr(className.c_str(), "PlayerCharacter") || (strstr(className.c_str(), "PlayerControllertSl") || (strstr(className.c_str(), "_PlayerPawn_TPlanAI_C")|| (strstr(className.c_str(), "CharacterModelTaget")|| (strstr(className.c_str(), "FakePlayer_AIPawn")!= 0 && moduleControl.mainSwitch.playerStatus)) )))) {
                    ...
                    bool bDead = Read<bool>(objectAddr + 0xdcc);
                    float hp = Read<float>(objectAddr + PubgOffset::ObjectParam::HpOffset);
                    if(bDead) continue;
