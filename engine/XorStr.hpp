...
#define XorEnc(str) xorstr(str).crypt_get()
...
int EncryptInt(const char *IntText) {
    int xvalue = std::stoi(IntText);
    return xvalue;
}

#define IntEnc(lol) EncryptInt(XorEnc(lol))


float EncryptFloat(const char *fstring) {
    float NumFlt = std::stof(fstring);
    return NumFlt;
}

#define FloatEnc(lol) EncryptFloat(XorEnc(lol))

uintptr_t EncryptUintptr_t(const char *charac) {
    uintptr_t value = (uintptr_t)  std::stoul(charac);;
    return value;
}

#define UPtrEnc(lol) EncryptUintptr_t(XorEnc(lol))

bool EncryptBoolean(const char *valueOfBool) {
    bool booleanValue;
    if(valueOfBool == "0" || valueOfBool == "false") {
        booleanValue = false;
    }else if(valueOfBool == "1" || valueOfBool == "true") {
        booleanValue = true;
    }
    return booleanValue;
}

#define BoolEnc(lol) EncryptBoolean(XorEnc(lol))
